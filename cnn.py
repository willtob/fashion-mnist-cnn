...
class Net(nn.Module):
    def __init__(self, num_classes):
        ...
        self.feature_extractor = nn.Sequential(
            ...
            # 32 * 28 * 28
            nn.ELU(),                                    # non-linear, doesn't ignore negative gradients
            nn.MaxPool2d(kernel_size=2),                 # takes max from each 2x2 region (halves spatial dims)
            # 32 * 14 * 14
            ...
            # 64 * 7 * 7
            nn.Flatten(),                                # 3D (C, H, W) -> 1D vector for linear layer
            # output: 3136 (64 * 7 * 7)
        )
...
# feed test data — no shuffle needed for evaluation
